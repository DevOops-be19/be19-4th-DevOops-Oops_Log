...
const handleFindPw = async () => {
  ...
  if (!memberId.value || !username.value || !email.value || !birth.value) {
  ...
  try{
    ...
    toastStore.showToast("이메일로 인증번호를 전송했습니다.");
    router.push(`/verify-code/${email.value}`)
  }catch(error){
...
<template>
  <div class="login-container" :class="{ entered }">
    ...
    <div class="login-form">
      ...
      <div class="form-group">
        <button @click.stop="handleFindPw" class="find-btn">비밀번호 찾기</button>
      </div>
    </div>
  </div>
</template>
